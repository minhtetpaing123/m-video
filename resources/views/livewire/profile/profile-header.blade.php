...
<div class="relative bg-white dark:bg-[#1e2235] transition-colors duration-200"
     x-data="{ 
    ...
    <div class="px-4 pb-3">
        <div class="flex justify-between items-end -mt-16 mb-3">
            ...
            <div class="relative w-28 h-28 flex-shrink-0">
                ...
                {{-- Inner Circle Box --}}
                <div class="w-full h-full rounded-full border-4 border-white dark:border-[#121420] bg-gray-200 dark:bg-gray-800 overflow-hidden shadow-2xl relative">
                    @if ($newAvatar)
                ...
                @if(auth()->check() && auth()->id() === $user->id)
                    <label class="absolute bottom-1 right-0 bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white p-2 rounded-full border-2 border-white dark:border-[#121420] cursor-pointer shadow-xl transition z-10 active:scale-95 flex items-center justify-center">
                ...
                <span x-show="isOnline && deviceOnline" 
                      class="absolute bottom-1 {{ $isOwner ? 'right-9' : 'right-1' }} w-4 h-4 bg-green-500 border-2 border-white dark:border-[#121420] rounded-full z-20"
                      x-cloak>
                    <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                </span>
                <span x-show="!isOnline || !deviceOnline" 
                      class="absolute bottom-1 {{ $isOwner ? 'right-9' : 'right-1' }} w-4 h-4 bg-gray-400 border-2 border-white dark:border-[#121420] rounded-full z-20"
                      x-cloak>
                </span>
            </div>
            ...
            <div class="flex items-center gap-2 mb-1">
                @if(auth()->check() && auth()->id() === $user->id)
                    ...
                    {{-- ✅ Modal အစား Settings Page Link သို့ တိုက်ရိုက်သွားရန် ပြောင်းထားပါသည် --}}
                    <a href="{{ Route::has('profile.edit') ? route('profile.edit') : url('/profile/edit') }}" 
                       class="px-4 py-1.5 bg-gray-100 hover:bg-gray-200 border border-gray-300 text-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 dark:border-gray-700 dark:text-gray-200 font-semibold text-xs rounded-lg flex items-center gap-1.5 transition">
                ...
                @else
                    ...
                    <button wire:click="toggleFollow" class="px-4 py-1.5 font-semibold text-xs rounded-lg flex items-center gap-1.5 shadow-md transition {{ $isFollowing ? 'bg-gray-200 hover:bg-gray-300 text-gray-800 border border-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 dark:border-gray-600' : 'bg-blue-600 hover:bg-blue-500 text-white' }}">
        ...
        {{-- Name & Username --}}
        <div class="mt-1">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight flex items-center gap-1.5">
                {{ $user->name }}
            </h1>
            
            <div class="flex items-center gap-2 mt-0.5">
                <p class="text-xs text-gray-500 dark:text-gray-400 font-medium">@ {{ $user->username ?? 'user' }}</p>

                {{-- Dynamic Online Status Text --}}
                <span class="text-xs text-gray-400 dark:text-gray-500">•</span>
                <span x-show="isOnline && deviceOnline" class="text-xs font-semibold text-green-500 dark:text-green-400" x-cloak>
                    Active now
                </span>
                <span x-show="!isOnline || !deviceOnline" class="text-xs text-gray-500 dark:text-gray-400" x-cloak>
        ...
        <div class="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
            <button wire:click="$dispatch('openFollowList', { userId: {{ $user->id }}, type: 'followers' })" 
                    class="flex items-center gap-1 hover:underline hover:text-blue-500 dark:hover:text-blue-400 transition group cursor-pointer">
                <strong class="text-gray-900 dark:text-white text-sm group-hover:text-blue-500 dark:group-hover:text-blue-400 transition">{{ $this->followersCount }}</strong>
                <span>Followers</span>
            </button>
            <button wire:click="$dispatch('openFollowList', { userId: {{ $user->id }}, type: 'following' })" 
                    class="flex items-center gap-1 hover:underline hover:text-blue-500 dark:hover:text-blue-400 transition group cursor-pointer">
                <strong class="text-gray-900 dark:text-white text-sm group-hover:text-blue-500 dark:group-hover:text-blue-400 transition">{{ $this->followingCount }}</strong>
                <span>Following</span>
            </button>
        </div>

        @if($user->bio)
            <p class="text-xs text-gray-700 dark:text-gray-300 mt-2.5 font-normal leading-relaxed">
                {{ $user->bio }}
            </p>
        @endif
    </div>
